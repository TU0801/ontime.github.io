// UI サウンドスケープの実装本体。ambient.ts から dynamic import され、
// facade (ui-sounds.ts) に playUiSound を register することで有効化される。
// よって「ambient が ON か」のチェックは呼び出し元の facade 側で担保される。
// Tone の既存 context を共有するので、新たな AudioContext は作らない。

import * as Tone from 'tone';

import type { SoundKind } from './ui-sounds';
